...
function changeProductField(data) {
    ...
    var product_price_list = data["price"]
    
    var p = 0
    
    for (var i = 0; i < length; i++) {
        var product_element = row_element[i].children
        if(product_element[0].value != 0) {
            product_element[2].value = product_price_list[p]
            p++
        }
    }
}
